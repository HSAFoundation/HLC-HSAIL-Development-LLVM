...
define void @test_activelanecount_u32_imm(i32 addrspace(1)* %out) #0 {
  %tmp = call i32 @llvm.HSAIL.activelanecount.u32.b1(i1 true) #0
  store i32 %tmp, i32 addrspace(1)* %out
  ret void
}

; HSAIL-LABEL: {{^}}prog function &test_activelanecount_width_u32(
; HSAIL: activelanecount_width(WAVESIZE)_u32_b1 {{\$s[0-9]+}}, {{\$c[0-9]+}};
define void @test_activelanecount_width_u32(i32 addrspace(1)* %out, i32 %x) #0 {
  %cmp = icmp eq i32 %x, 0
