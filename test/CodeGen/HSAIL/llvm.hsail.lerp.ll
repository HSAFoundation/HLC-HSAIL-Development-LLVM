...
declare i32 @llvm.hsail.lerp(i32, i32, i32) #0
...
define i32 @test_lerp_i32(i32 %x, i32 %y, i32 %z) #0 {
  %val = call i32 @llvm.hsail.lerp(i32 %x, i32 %y, i32 %z) #0
  ret i32 %val
}
...
define i32 @test_lerp_i32_rii(i32 %x) #0 {
  %val = call i32 @llvm.hsail.lerp(i32 %x, i32 16777216, i32 2) #0
  ret i32 %val
}
...
define i32 @test_lerp_i32_rir(i32 %x, i32 %y) #0 {
  %val = call i32 @llvm.hsail.lerp(i32 %x, i32 1, i32 %y) #0
  ret i32 %val
}
...
define i32 @test_lerp_i32_rri(i32 %x, i32 %y) #0 {
  %val = call i32 @llvm.hsail.lerp(i32 %x, i32 %y, i32 7) #0
  ret i32 %val
}
...
define i32 @test_lerp_i32_iri(i32 %x) #0 {
  %val = call i32 @llvm.hsail.lerp(i32 342421, i32 %x, i32 1106411520) #0
  ret i32 %val
}
...
define i32 @test_lerp_i32_iir(i32 %x) #0 {
  %val = call i32 @llvm.hsail.lerp(i32 256, i32 65536, i32 %x) #0
  ret i32 %val
}
...
define i32 @test_lerp_i32_iii() #0 {
  %val = call i32 @llvm.hsail.lerp(i32 3, i32 1234, i32 11) #0
  ret i32 %val
}

; HSAIL-LABEL: {{^}}prog function &test_legacy_lerp_i32(
; HSAIL: lerp_u8x4 {{\$s[0-9]+}}, {{\$s[0-9]+}}, {{\$s[0-9]+}}, {{\$s[0-9]+}};
define i32 @test_legacy_lerp_i32(i32 %x, i32 %y, i32 %z) #0 {
