...
; HSAIL-LABEL: {{^}}prog function &test_legacy_nsqrt_f32(
; HSAIL: nsqrt_f32 {{\$s[0-9]+}}, {{\$s[0-9]+}};
define float @test_legacy_nsqrt_f32(float %x) #0 {
  %sqrt = call float @llvm.HSAIL.nsqrt.f32(float %x) #0
  ret float %sqrt
}

; HSAIL-LABEL: {{^}}prog function &test_legacy_nsqrt_f64(
; HSAIL: nsqrt_f64 {{\$d[0-9]+}}, {{\$d[0-9]+}};
define double @test_legacy_nsqrt_f64(double %x) #0 {
  %sqrt = call double @llvm.HSAIL.nsqrt.f64(double %x) #0
  ret double %sqrt
}
