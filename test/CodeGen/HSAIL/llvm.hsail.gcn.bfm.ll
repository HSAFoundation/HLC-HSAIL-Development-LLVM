...
declare i32 @llvm.hsail.gcn.bfm(i32, i32) #0
...
define i32 @test_bfm_i32(i32 %x, i32 %y, i32 %z) #0 {
  %val = call i32 @llvm.hsail.gcn.bfm(i32 %x, i32 %y) #0
  ret i32 %val
}
...
define i32 @test_bfm_i32_ri(i32 %x) #0 {
  %val = call i32 @llvm.hsail.gcn.bfm(i32 %x, i32 7) #0
  ret i32 %val
}
...
define i32 @test_bfm_i32_ir(i32 %x) #0 {
  %val = call i32 @llvm.hsail.gcn.bfm(i32 3, i32 %x) #0
  ret i32 %val
}
...
define i32 @test_bfm_i32_ii() #0 {
  %val = call i32 @llvm.hsail.gcn.bfm(i32 6, i32 9) #0
  ret i32 %val
}

; HSAIL-LABEL: {{^}}prog function &test_legacy_bfm_i32(
; HSAIL: gcn_bfm_b32 {{\$s[0-9]+}}, {{\$s[0-9]+}}, {{\$s[0-9]+}};
define i32 @test_legacy_bfm_i32(i32 %x, i32 %y, i32 %z) #0 {
