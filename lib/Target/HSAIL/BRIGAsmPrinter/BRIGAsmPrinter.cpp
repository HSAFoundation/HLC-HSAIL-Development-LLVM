...
BRIGAsmPrinter::BRIGAsmPrinter(TargetMachine &TM,
                               std::unique_ptr<MCStreamer> Streamer)
    : HSAILAsmPrinter(TM, std::move(Streamer)),
      Subtarget(nullptr), FuncArgsStr(),
      FuncRetValStr(), retValCounter(0), paramCounter(0),
      mTM(reinterpret_cast<HSAILTargetMachine *>(&TM)),
      TII(nullptr), mMeta(new HSAILKernelManager(mTM)),
      mMFI(nullptr), m_bIsKernel(false), brigantine(bc), mDwarfStream(nullptr),
      mBrigStream(nullptr), mDwarfFileStream(nullptr),
      mBuffer(0) {
  // Obtain DWARF stream.
  BRIGDwarfStreamer &DwarfStreamer = cast<BRIGDwarfStreamer>(*OutStreamer);
...
bool BRIGAsmPrinter::runOnMachineFunction(MachineFunction &lMF) {
  ...
  Subtarget = &lMF.getSubtarget<HSAILSubtarget>();
...
void BRIGAsmPrinter::EmitBasicBlockStart(const MachineBasicBlock &MBB) const {
...
void BRIGAsmPrinter::EmitStartOfAsmFile(Module &M) {
  ...
  const DataLayout &DL = getDataLayout();
  bool IsLargeModel = (DL.getPointerSize(HSAILAS::GLOBAL_ADDRESS) == 8);
  ...
  brigantine.module("&__llvm_hsail_module", BRIG_VERSION_HSAIL_MAJOR,
                    ...
                    IsLargeModel ? BRIG_MACHINE_LARGE : BRIG_MACHINE_SMALL,
                    IsFullProfile ? BRIG_PROFILE_FULL : BRIG_PROFILE_BASE,
...
void BRIGAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  if (mDwarfStream) {
    ...
    if (MMI->hasDebugInfo()) {
      ...
      HSAIL_ASM::SRef data = makeSRef(mDwarfStream->getData());
