...
namespace llvm {
  ...
  public:
    ...
    explicit DIPGVariable(const MDNode* N, HSAILAS::AddressSpaces addressSpace): DIGlobalVariable(N), addrSpace(addressSpace) {}
  ...
  protected:
    ...
    virtual void collectVariableInfo(const MachineFunction *MF,
...
#ifdef BRIG_DWARF_DEMANGLE_PRIVATE_VARIABLES
    void addScopeGroupVariable(const std::string& demangledName, uint64_t brigOffset);
    void addScopePrivateVariable(const std::string& demangledName, uint64_t brigOffset);
#else
  ...
  public:
    ...
    /// addGroupVariableMDNode - callback function for BRIGDwarfCompileUnit that notifies
