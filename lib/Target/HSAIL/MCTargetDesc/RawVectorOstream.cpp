...
namespace llvm {
  ...
  void RawVectorOstream::write_impl(const char *Ptr, size_t Size) {
    ...
    if(_position + Size > _data.size()) {
      ...
      size_t nChunks = (_position + Size + allocation_chunk - 1) / allocation_chunk;
  ...
  const std::vector<char>& RawVectorOstream::getData() const {
    return _data;
  }

  RawVectorOstream::RawVectorOstream(raw_ostream* other) : _data(allocation_chunk), _position(0), _other(other) { 
  }

  RawVectorOstream::~RawVectorOstream() {
    // make sure that releaseStream has been called before RawVectorOstream is deleted 
    assert(!_other);
#if 0
    if(_other) { 
      flush();
  ...
  raw_ostream* RawVectorOstream::getOtherStream() {
    return _other;
  }
