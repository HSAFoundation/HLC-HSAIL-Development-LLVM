...
namespace llvm {
  ...
  protected:
    ...
    virtual void addDebugLocOffset(const DbgVariable *Var, DIE* VariableDie, unsigned int Offset);

  public:

    BRIGDwarfCompileUnit(unsigned UID, DIE *D, DICompileUnit CU, BRIGAsmPrinter *A, BRIGDwarfDebug *DW);
    virtual ~BRIGDwarfCompileUnit();

    virtual void addVariableAddress(DbgVariable *&DV, DIE *Die, MachineLocation Location);
