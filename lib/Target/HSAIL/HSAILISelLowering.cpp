    ...
    if (ConstantSDNode *AndMask =
      ...
      return DAG.getNode(ISD::AND, SDLoc(Opr1), Opr1.getValueType(),
                         DAG.getNode(ISD::INTRINSIC_WO_CHAIN, dl, VT,
                                     ...
                                     Opr0, Opr0, Opr2),
                         DAG.getConstant(and_mask, dl, MVT::i32));
    }
  }
  ...
  if (SHR && (IID == HSAILIntrinsic::HSAIL_bitalign_b32)) {
    ...
    if ((shr_val & 7U) == 0)
      return DAG.getNode(
          ...
          DAG.getConstant(HSAILIntrinsic::HSAIL_bytealign_b32, dl, MVT::i32),
          Opr0, Opr1, DAG.getConstant(shr_val >> 3U, dl, MVT::i32));
  }
  return SDValue();
}
...
SDValue HSAILTargetLowering::getArgLoad(SelectionDAG &DAG, SDLoc SL, EVT ArgVT,
  ...
  SDValue PtrOffs =
    DAG.getNode(ISD::ADD, SL, PtrVT, Ptr, DAG.getConstant(Offset, SL, PtrVT));

  const SDValue Ops[] = {
    Chain,
    PtrOffs,
    DAG.getTargetConstant(Width, SL, MVT::i32),
    DAG.getTargetConstant(IsRetArgLoad, SL, MVT::i1),
    DAG.getTargetConstant(isSExt, SL, MVT::i1),
    InFlag
  };
...
SDValue HSAILTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  if (!retType->isVoidTy()) {
    ...
    const SDValue ArgDeclOps[] = {
      ...
      DAG.getTargetConstant(NElts, dl, PtrVT),
  ...
  VarOps.push_back(DAG.getTargetConstant(0, dl, MVT::i32));
  ...
  for (FunctionType::param_iterator pb = funcType->param_begin(),
    ...
    const SDValue ArgDeclOps[] = {
        StParamValue,
        DAG.getTargetConstant(BT, dl, MVT::i32),
        DAG.getTargetConstant(NElts, dl, ArgPtrVT),
        DAG.getTargetConstant(Align, dl, MVT::i32),
        Chain,
        InFlag
    };
...
SDValue HSAILTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case HSAILIntrinsic::HSAIL_segmentp_global: {
    return DAG.getNode(
      HSAILISD::SEGMENTP, SL, MVT::i1,
      DAG.getTargetConstant(HSAILAS::GLOBAL_ADDRESS, SL, MVT::i32),
      DAG.getTargetConstant(0, SL, MVT::i1), Op.getOperand(1));
  }
  case HSAILIntrinsic::HSAIL_segmentp_local: {
    return DAG.getNode(
      HSAILISD::SEGMENTP, SL, MVT::i1,
      DAG.getTargetConstant(HSAILAS::GROUP_ADDRESS, SL, MVT::i32),
      DAG.getTargetConstant(0, SL, MVT::i1), Op.getOperand(1));
  }
...
SDValue HSAILTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
  ...
  case HSAILIntrinsic::HSAIL_activelanepermute_b32: {
    ...
    const SDValue Ops[] = {
        Op.getOperand(0),                                  // Chain
        DAG.getTargetConstant(BRIG_WIDTH_1, SL, MVT::i32), // width
  ...
  case HSAILIntrinsic::HSAIL_activelanepermute_width_b32: {
    ...
    const SDValue Ops[] = {
        Op.getOperand(0),                                         // Chain
        DAG.getTargetConstant(BRIG_WIDTH_WAVESIZE, SL, MVT::i32), // width
        Op.getOperand(2),                                         // src0
        Op.getOperand(3),                                         // src1
        Op.getOperand(4),                                         // src2
        Op.getOperand(5)                                          // src3
...
HSAILTargetLowering::lowerSamplerInitializerOperand(SDValue Op,
  ...
  SDValue Ops[] = {
    ...
    DAG.getTargetConstant(1, SL, MVT::i1), // Const
...
SDValue HSAILTargetLowering::LowerROTL(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue IntrID
    = DAG.getTargetConstant(HSAILIntrinsic::HSAIL_bitalign_b32, dl, MVT::i32);
  ...
  return DAG.getNode(
    ISD::INTRINSIC_WO_CHAIN, dl, VT,
      IntrID,
      src0, src0,
      shift ? DAG.getConstant(32 - (shift->getZExtValue() & 31), dl, MVT::i32)
            : DAG.getNode(ISD::SUB, dl, VT, DAG.getConstant(0, dl, VT), src1));
}
...
SDValue HSAILTargetLowering::LowerROTR(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue IntrID
    = DAG.getTargetConstant(HSAILIntrinsic::HSAIL_bitalign_b32, dl, MVT::i32);
  return DAG.getNode(ISD::INTRINSIC_WO_CHAIN, dl, VT, IntrID, src0, src0, src1);
}
...
SDValue HSAILTargetLowering::LowerBSWAP(SDValue Op, SelectionDAG &DAG) const {
  ...
  const SDValue opr0 = DAG.getNode(
      ISD::INTRINSIC_WO_CHAIN, dl, VT,
      DAG.getConstant(HSAILIntrinsic::HSAIL_bytealign_b32, dl, MVT::i32),
      src, src, DAG.getConstant(3, dl, MVT::i32));
  const SDValue opr1 = DAG.getNode(
      ISD::INTRINSIC_WO_CHAIN, dl, VT,
      DAG.getConstant(HSAILIntrinsic::HSAIL_bytealign_b32, dl, MVT::i32),
      src, src, DAG.getConstant(1, dl, MVT::i32));
  return DAG.getNode(
      ISD::INTRINSIC_WO_CHAIN, dl, VT,
      DAG.getTargetConstant(HSAILIntrinsic::HSAIL_bitselect_u32, dl, MVT::i32),
      DAG.getConstant(0x00ff00ff, dl, VT), opr0, opr1);
}
...
static SDValue getMemFenceImpl(SDValue Chain, SDLoc SL, unsigned MemoryOrder,
  ...
  const SDValue Ops[] = {
    Chain,
    CurDAG.getTargetConstant(HSAILIntrinsic::HSAIL_memfence, SL, MVT::i64),
    CurDAG.getConstant(MemoryOrder, SL, MVT::i32),
    CurDAG.getConstant(GlobalMemoryScope, SL, MVT::i32),
    CurDAG.getConstant(GroupMemoryScope, SL, MVT::i32),
    CurDAG.getConstant(ImageMemoryScope, SL, MVT::i32)
  };
