...
public:
  explicit HSAILDAGToDAGISel(TargetMachine &TM)
      : SelectionDAGISel(TM), Subtarget(&TM.getSubtarget<HSAILSubtarget>()) {}
...
static SDValue getBRIGMemorySegment(SelectionDAG *CurDAG, unsigned memSeg) {
  unsigned BRIGMemSegment;
  switch (memSeg) {
  case HSAILAS::GLOBAL_ADDRESS:
    BRIGMemSegment = BRIG_SEGMENT_GLOBAL;
    break;
  case HSAILAS::GROUP_ADDRESS:
    BRIGMemSegment = BRIG_SEGMENT_GROUP;
    break;
  case HSAILAS::FLAT_ADDRESS:
    BRIGMemSegment = BRIG_SEGMENT_FLAT;
    break;
  default:
    llvm_unreachable("unexpected memory segment ");
  }
  SDValue memSegSD =
      CurDAG->getTargetConstant(BRIGMemSegment, MVT::getIntegerVT(32));
  return memSegSD;
}
...
SDNode *HSAILDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::GlobalAddress: {
    ...
    const SDValue Ops[] = {CurDAG->getTargetConstant(AS, MVT::i32),
...
bool HSAILDAGToDAGISel::SelectLoadAddr(SDNode *ParentLoad, SDValue Addr,
  ...
  ModifierMask =
      CurDAG->getTargetConstant(0, MVT::i32); // TODO: Set if invariant
  return true;
}
...
bool HSAILDAGToDAGISel::SelectAtomicAddr(SDNode *ParentAtomic, SDValue Addr,
  ...
  Scope =
      CurDAG->getTargetConstant(getBrigMemoryScope(SyncScope, AS), MVT::i32);
  Equiv = CurDAG->getTargetConstant(0, MVT::i32);
...
SDNode *HSAILDAGToDAGISel::SelectArgLd(MemSDNode *Node) const {
  ...
  SDValue Ops[10] = {
      Base, Reg, Offset, CurDAG->getTargetConstant(BT, MVT::i32), // TypeLength
...
SDNode *HSAILDAGToDAGISel::SelectArgSt(MemSDNode *Node) const {
  ...
  SDValue Ops[9] = {
      Node->getOperand(1), Base, Reg, Offset,
